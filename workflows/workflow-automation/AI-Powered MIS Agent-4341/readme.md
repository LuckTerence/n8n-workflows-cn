...
> 分类：工作流自动化 | 适配等级：A（需替换Pinecone/Google Sheets/Google Drive/Gmail)（基本改完，配置 API Key 应该就能跑）
> 原始来源：https://n8nworkflows.xyz/workflows/4341
...
| Address | 数据设置 |
| customer_info | 数据设置 |
| Drive Folders URL | 数据设置 |
| Drive Folders URL1 | 数据设置 |
| Daily Sales Folder | Google Drive |
| Address Folder | Google Drive |
| customer_info folder | Google Drive |
| Merge | 数据合并 |
| Master | Google Drive |
| Wait | 等待 |
...
| Edit Fields12 | 数据设置 |
| Code8 | Code |
| Operation Classifier | 文本分类器 |
| JSON PARSER (Join) | 信息提取器 |
| OpenAI Chat Model7 | OpenAI Chat Model |
| Google Sheets8 | Google Sheets 工具 |
| Call n8n Workflow Tool1 | 工作流工具 |
...
| Edit Fields9 | 数据设置 |
| Edit Fields11 | 数据设置 |
| Google Drive Trigger3 | Google Drive 触发器 |
| Edit Fields13 | 数据设置 |
...
AI-Powered MIS Agent（AI 增强)定时触发、Chat 消息触发，通过 邮箱 + 在线表格 + 数据库 + HTTP API 实现自动化。

定时触发、Chat 消息触发，通过 邮箱 + 在线表格 + 数据库 + HTTP API 实现自动化。
